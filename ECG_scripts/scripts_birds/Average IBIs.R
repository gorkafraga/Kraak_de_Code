...
rm(list=ls())
dirinput <- "Z:/fraga/ECG_ABMP/ECG_Kraak_Adults/ECG_adults/bva_output/quartiles"
diroutput <- "Z:/fraga/ECG_ABMP/ECG_Kraak_Adults/"
## Setting WD to correct path
setwd(dirinput)

condition <-"Fout_q1"
files <- list.files(path=dirinput,pattern=paste('s*EditMarkers_',condition,'_IBIs.txt',sep=''))
