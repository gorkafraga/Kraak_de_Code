...
diroutput = 'Z:\fraga\EEG_ABMP\Kraak_analysis\Kraak EEG\resting-state EEG\output_pre\Edit Channels\FFT spectra\30 seg power Excl 8,20,38,102\';
...
for s = 1:length(ppnr); %length(files);
  ...
  % import data      
    ...
    for ch = 1:64; 
        fftAbs(s,:,ch,seg)  = (1/(srate*N))*abs(fft(data(:,ch))).^2;
    end 
...
% Include IAF info
     ...
     xlabel('Frequencies (Hz)'), ylabel('Power mV^2/Hz');
