...
% =====================================================================
dirinput=  'Z:\fraga\EEG_ABMP\Kraak_analysis\Kraak EEG\resting-state EEG\output_pre\s107\Brainwave_output';
diroutput = 'Z:\fraga\EEG_ABMP\Kraak_analysis\Kraak EEG\resting-state EEG\output_pre\s107\Brainwave_output';
cd (dirinput)
%% define filenames
band= 'Alpha';
group = '_';
fileinput = ['Matrix_s107',group,band,'_allinone.txt'];
...
%find matrices for each subject and segment
headersidx = find(~cellfun('isempty',strfind(data(:,2),'4sSeg')));
for h = 1:length(headersidx); 
     data(headersidx(h),2:3) = strsplit(data{headersidx(h),2},'_4sSeg_'); % split string specifiying subject and segment , now in 2 columns
end
subjects = unique(data(headersidx,2)); % In the file headers the second column contained subject info.
segments = unique(data(headersidx,3)); % In the file headers the second column contained segment info.
...
for ss= 1:length(subjects);
    segidx = strmatch(subjects(ss),data(:,2),'exact');
    for seg=1:length(segidx);
       temp  = data(segidx(seg)+2:segidx(seg)+2+63,:); %matrix for segment(seg) and subject(ss)
