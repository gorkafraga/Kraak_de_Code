...
% =========================================================================
clear all; close all; 
dirinput = 'X:\fraga\EEG_ABMP\Kraak_analysis\Kraak EEG\resting-state EEG\output_pre\Edit Channels\ICs rej';
diroutput = 'X:\fraga\EEG_ABMP\Kraak_analysis\Kraak EEG\resting-state EEG\output_pre\Edit Channels\ICs rej';
cd (dirinput);
% list files
filelist = dir('*ICsRej.set');  
files = {filelist.name}; 
% define inputs
header= {'ppnr','badSegs_4sec','ICs_kept', 'ICs_rej','Chans_kept'} ;
...
countEp = []; countICsKept = [];countICsRej=[]; countChans = []; 
 for f = 1:length(files);
                ...
                countChans(f) = length(EEG.chanlocs);
end
