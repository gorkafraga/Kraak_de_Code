...
% - Save in xls
clear all
dirinput = 'Z:\fraga\EEG_ABMP\Kraak_analysis\Kraak EEG\resting-state EEG\output_pre\Edit Channels\output_SARA_interp';
diroutput = 'Z:\fraga\EEG_ABMP\Kraak_analysis\Kraak EEG\resting-state EEG\output_pre\Edit Channels\output_SARA_interp';
cd(dirinput); 
%% read data (1st line header not included)
group = 'Close';
band= 'delta';
suffix = '_interp_avgRef.txt';
fileinput = [group,'_',band,suffix];
...
for row = 1:length(data);
    tmp(row,:) = strsplit(data{row,1},'_4sSeg');
end; data(:,1:2) = tmp;
