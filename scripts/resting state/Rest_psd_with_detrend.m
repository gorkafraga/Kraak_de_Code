...
dirinput = 'Z:\fraga\EEG_ABMP\Kraak_analysis\Kraak EEG\resting-state EEG\output_pre\Edit Channels\ASCIIs avgRef fixed\' ;
diroutput = 'Z:\fraga\EEG_ABMP\Kraak_analysis\Kraak EEG\resting-state EEG\output_pre\Edit Channels\PSD avgRef fixed detrend\';

%% list files and input parameters 
cd (dirinput);
list = dir('*Seg_AvgRef*.txt');                
files = {list.name}; 
% Number of segments
segments = 1:30; 
% signal parameters
...
% Plot y-axis range
ylim1 = [0 100];
ylim2 = [0 100]; %for grand averages 
xlim = 49;
...
for s = 1:length(ppnr); %length(files);
        ...
        for seg = 1:length(segments);                                                                                          %START SEGMENT LOOP
                  fileinput = [ppnr{s},'_4sSeg_avgRef_',num2str(segments(seg)),'.txt'];
                if ~ exist([ppnr{s},'_4sSeg_avgRef_',num2str(segments(seg)),'.txt']);
                       disp([[ppnr{s},'_4sSeg_avgRef_',num2str(segments(seg)),'.txt'],' not available']);
               else
              % import data      
                ...
                %% Spectrum of segment for each channel
                for ch = 1:64; 
                    psd_db(s,:,ch,seg)= 10*log10((abs(fft(data(:,ch))).^2)./size(data,1));%"10*log10((abs(fft(data)).^2)./size(data,1))"  
                        p = polyfit(fax_Hz,psd_db(s,:,ch,seg),1);  % degree = 1? 
                        f1 = polyval(p,fax_Hz);
                      psd_db(s,:,ch,seg) = psd_db(s,:,ch,seg)-f1;
                end 
                %% Spectrum averaged  across channels
                  psd_db_chansMean(s,:,seg) = mean(psd_db(s,:,:,seg),3);
                %% Spectrum averaged across clusters 
                    for clus = 1:length(clusters);
                      ...
                      psd_db_clusters(s,:,seg,clus) = mean(psd_db(s,:,chanidx,seg),3); 
                      clear chanidx
                    end    
                end
         end                                                                                                        % close segment loop
%% Plot segment spectrum;  
%         figure;
%         % plot(hz,2*(totalFabs(s,1:length(hz),seg)),'k- .');title([ppnr{s},'mean Spectrum segment',num2str(seg)]); 
%         plot(hz,(psd_db_chansMean(s,1:length(fax_Hz),seg)),'b-'); title([ppnr{s},'mean Spectrum segment',num2str(seg)]); 
...
%        close gcf
    ...
    psd_db_chansMean(psd_db_chansMean == 0) = NaN; % replace 0s of the missing segments by NaN to be ignored in the mean
    segsAvail = length(find(~isnan(psd_db_chansMean(s,1,:)))); % info of how many segments current subject had 
  %% Peak detection for the average spectra across channels and segments 
   smean = nanmean(psd_db_chansMean(s,:,:),3);
...
% Include IAF info
     ...
     xlabel('Frequencies (Hz)'), ylabel('PSD (db)');
...
%% save
       ...
       cd(dirinput)
       close gcf 
       group_psd_db_chansMean(s,:) = nanmean(psd_db_chansMean(s,:,:),3);
       clear  data 
  %% Plot and save spectrum averaged over segments for each CLUSTER
      mean_psd_db_clusters  = mean(psd_db_clusters(s,:,:,:),3); % average across segments of each cluster (indexed by 4th dim)
      figure;
   for clus= 1:length(clusters)
     data2use = squeeze(mean_psd_db_clusters);
...
tmpmean = nanmean(group_psd_db_chansMean(:,:,:),3);
...
cd (diroutput);
for clus = 1:length(clusters);
    cntrldata2plot =  mean(psd_db_clusters(find(ppnrnum<100),:,:,clus),3);
        cntrldata2plot = mean(cntrldata2plot,1);
    dysdata2plot =  mean(psd_db_clusters(find(ppnrnum>100),:,:,clus),3);
...
%% Plot 
group_psd_db_cluster = squeeze(mean(psd_db_clusters(:,:,:,:),3));
for clus= 1:length(clusters)
   tmp = [ppnr',num2cell(group_psd_db_cluster(:,1:length(fax_Hz),clus))];
     xlswrite(['fft_',clusters{clus},'.xlsx'],tmp);
end
  %xlswrite('fft_chanMeans.xls',[ppnr',num2cell(nanmean(group_psd_db_chansMean(:,1:length(hz),:),3))]);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
