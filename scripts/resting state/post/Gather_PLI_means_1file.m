...
for ss= 1:length(subjects);
    ...
    for seg=1:length(subjidx);
       tmp  = data(subjidx(seg)+2:subjidx(seg)+2+63,:); %matrix for segment(seg) and subject(ss)
        segdatanum = zeros(64,64);
        for r= 1:size(tmp,1);
            for c =1:size(tmp,2);
                segdatanum(r,c)= str2num(tmp{r,c});
            end
        end
...
names = cell2mat(subjects);
names = str2num(names(:,2:4));
header_clusters = ['subject',strcat('Post_',[band,'_'],clusters)];
